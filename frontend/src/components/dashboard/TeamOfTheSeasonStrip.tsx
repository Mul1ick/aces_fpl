...
export const TeamOfTheSeasonStrip: React.FC<TeamOfTheSeasonStripProps> = ({ team }) => {
  ...
  if (!team || !team.starting || team.starting.length === 0) {
    return (
      <Card className="border-black border-2 bg-white mt-8 shadow-sm w-full">
  ...
  return (
    <div className="w-full mt-12 mb-8">
      ...
      {/* --- STATIC FLEX WRAP CONTAINER (No Scroll) --- */}
      <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
        {sortedPlayers.map((player) => (
          <div 
            key={player.id} 
            className="flex-shrink-0 w-32 sm:w-36 bg-black text-white rounded-xl overflow-hidden shadow-xl border-2 border-black relative group transition-transform hover:-translate-y-1 duration-300"
          >
            {/* Card Body */}
